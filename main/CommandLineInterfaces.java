...
import main.resources.LiftRequestJSONUtil;
...
public class CommandLineInterfaces {
    public static void batchProcessing(Scanner sc, LiftManager liftManager) throws IOException {
        ...
        Map<Integer, List<LiftRequestJSONUtil>> requests = ResourceUtility.loadLiftRequests
                ("C:\\Users\\ngaddamanugu\\IdeaProjects\\liftSimulator\\main" +
                        "\\resources\\liftRequests.json");
        ...
        while (true) {
            try {
                ...
                if (requests.containsKey(selectedBatchId)) {
                    for (LiftRequestJSONUtil request : requests.get(selectedBatchId)) {
                        try{
